	...
	*qsp = 0;
	...
	donesp+=ssds_get_vers(dbfd, donesp, NULL);
      ...
      if(strncmp(tokp->tok,"@@gv.",5) == 0) {
        ...
        res = 0;
	sp += tokp->tlen;
	res = ssds_item_key(dbfd, tokp->tok, donesp, "value");
        if(res > 0) donesp += res;
      } else
      if(strncmp(tokp->tok,"@@gn.",5) == 0) {
        ...
        res = 0;
	sp += tokp->tlen;
	res = ssds_item_key(dbfd, tokp->tok, donesp, "name");
        if(res > 0) donesp += res;
      } else
      if(strncmp(tokp->tok,"@@gi.",5) == 0) {
        int res;
        res = 0;
	sp += tokp->tlen;
	res = ssds_item_key(dbfd, tokp->tok, donesp, "id");
        if(res > 0) donesp += res;
      } else
      if(strncmp(tokp->tok,"@@gs.",5) == 0) {
        int res;
        res = 0;
	sp += tokp->tlen;
	res = ssds_item_key(dbfd, tokp->tok, donesp, "num");
        if(res > 0) donesp += res;
      } else
...
int find_tokens(char *sp, int bytes)
{
  ...
  while (!done) {
    ...
    if(stok && *stok) {
      if(debug)
        printf("found token start at %ld (%c) ", stok-sp, *stok);
      tstart = stok;
      stok += 2;
      stok=strstr(stok,"@@");
      if(stok && *stok) {
	...
	tlen = tend-tstart;
	if(debug)
	  printf("end at %ld (%c) ", stok-sp, *stok);
